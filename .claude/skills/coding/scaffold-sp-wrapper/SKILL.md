...
- **Output file path** — full path to `<usecase>_functions.py`; skill is repo-agnostic
- **Use case name** — the string used as `use_case_name` default
...
**Dummy (semantic label — body is always `pass`):**
```python
@register_function("dummy_<concept>", "<Source Name>")
async def dummy_<concept>():
    pass
```

**Real orchestrator (calls one or more raw wrappers):**
```python
@register_function(
    "get_factual_data",
    "Factual Data Query",
...
- **Dummy functions must have distinct `source_name` values.** The template selector prompt deduplicates by source_name. Two dummies with the same source_name appear as one capability.
